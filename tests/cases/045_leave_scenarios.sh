...
auth_curl -X POST -d "name=Vacation&default_days_per_year=20&requires_approval=1" "http://localhost:8080/w/${TENANT_SLUG}/leave/settings" >/dev/null 2>&1 || true
...
auth_curl -X POST -d "start_date=$(date +%F)&end_date=$(date +%F)&days=1&leave_type_id=${LT_ID}&reason=Vacation" "http://localhost:8080/w/${TENANT_SLUG}/leave/request" >/dev/null 2>&1 || true
...
auth_curl -X POST -d "action=approve&review_note=OK" "http://localhost:8080/w/${TENANT_SLUG}/leave/review/${REQ_ID}" >/dev/null 2>&1 || true
